...
class ParticleFilterVisualizer:
    def __init__(self):
        ...
        # Room bounds (must match launch parameters)
        self.room_min = -1.5
        self.room_max =  1.5

        self.tag_positions = [
            (-0.274,  1.280), ( 0.263,  1.280),
            (-1.240, -0.719), (-1.240,  0.698),
            ( 1.280, -0.747), ( 1.280,  0.468),
            ( 0.647, -1.241), (-0.940, -1.240),
        ]

        rospy.Subscriber('/odom',             Odometry,         self._odom_cb)
        rospy.Subscriber('/pf_path',          Path,             self._pf_path_cb)
        rospy.Subscriber('/particles',        PoseArray,        self._particles_cb)
        rospy.Subscriber('/particle_weights', Float32MultiArray, self._weights_cb)
        rospy.Subscriber('/pf_neff',          Float32MultiArray, self._neff_cb)
    ...
    def _draw_room_boundary(self, img):
        tl = self.world_to_pixel(self.room_min, self.room_max)
        br = self.world_to_pixel(self.room_max, self.room_min)
