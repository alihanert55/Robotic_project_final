...
class CustomArucoTracker:
    def __init__(self):
        ...
        self.poses_pub = rospy.Publisher("/aruco_tracker/poses", PoseArray, queue_size=10)
    ...
    def image_callback(self, data):
        ...
        pose_array.header.frame_id = data.header.frame_id or "camera_rgb_optical_frame"
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                ...
                txt = f"ID:{self.target_id} Z:{tvec[2][0]:.2f}m"
                cv2.putText(out_img, txt,
                            (int(corners[i][0][0][0]), int(corners[i][0][0][1]) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Always publish — even an empty PoseArray tells the filter "nothing seen".
    ...
    def _build_pose(self, tvec, rvec) -> Pose:
        """Convert solvePnP output to a geometry_msgs/Pose in camera optical frame."""
        p = Pose()
