...
class ParticleFilterNode:
    def __init__(self):
        ...
        bounds = rospy.get_param('~room_bounds', [-0.62, -0.40, 0.62, 0.40])
        self.room_bounds = bounds  # [x_min, y_min, x_max, y_max]

        self.init_x     = rospy.get_param('~initial_x',     0.0)
        self.init_y     = rospy.get_param('~initial_y',     0.0)
        self.init_theta = rospy.get_param('~initial_theta', 0.0)
    ...
    def _initialize_particles(self, num):
        ...
        particles[:, 0] = self.init_x     + np.random.normal(0, 1.0, num)
        particles[:, 1] = self.init_y     + np.random.normal(0, 1.0, num)
        particles[:, 2] = self.init_theta + np.random.normal(0, 0.5, num)
        return particles
